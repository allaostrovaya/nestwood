...
flowchart TD
    ...
    new -->|"Зберегти план — акаунт уже є"| plan["plan · План<br/>загальні алерти → дні зі своїми алертами"]
...
> ⚠️ **Перебудова 2026-09-23 (дві за день).** Головний потік: **пошук → генерація → новий план → план → день** (вхід — аркуш поза основним флоу, у демо акаунт уже є). `route` став `new-plan`; `night`, `transport`, `today` — зони й стан `day`; `nights`, `lock-in`, `offline-pack` — зони `plan`; `field-notes`, `reviews` — `notes`. Друга перебудова: `changes` зник — кожен алерт веде туди, де зміну виправляють; `notify` — системний запит після входу; бронювання й квитки — аркуш `booking` поза основним флоу; у поході вкладка «Плани» відкриває сьогоднішній день. Назви екранів у потоках нижче перейменовані за цією відповідністю.
