...
flowchart TD
    ...
    FRESH --> plan["plan · План<br/>зверху «Що ще потрібно від тебе»"]
    QACT -->|ні| catalogue["catalogue · Куди можна піти<br/>рядок пошуку: країна → регіон · гнучкі дати · хто йде (людина / член DNT·STF) · тип ночівлі<br/>карта на весь екран + аркуш: найкраще в країні в місяць · класика Скандинавії"]

    catalogue --> QFIT{"Під ці фільтри щось є?"}
    QFIT -->|"ні"| NOFIT("empty на catalogue · напр. лютий — хижі зачинені<br/>виходи: інші дати · класика на літо")
    NOFIT -.-> catalogue
    QFIT -->|"пресет"| QGEN
    catalogue -->|"свій маршрут: тап старту й фінішу<br/>точки задані — станції, входи на мережу, хижі"| QGEN{"Є мережа для генерації?"}
    QGEN -->|ні| ENDGEN(["ЧЕСНИЙ КІНЕЦЬ · плану без мережі не існує<br/>бриф збережено, генерація стартує сама"])
    QGEN -->|так| GEN("new-plan · генерація — анімований екран<br/>стежки → ланцюжок ночей по графу хиж → навантаження → транспорт → погода → нотатки")

    GEN --> QSRC{"Критичне джерело відповіло?"}
    QSRC -->|ні| ERR("new-plan · помилка · плану немає, здогад не підставляємо<br/>[ Спробувати ще раз ]")
    QSRC -->|так| QOK{"Маршрут складається в ці дні?"}
    QOK -->|"ні"| CONF("new-plan · неможливий маршрут · зберегти не можна<br/>«між Sitasjaure й Vaisaluokta 38 км — не денний перехід»<br/>виходи: +1 день · інший фініш · намет")
    CONF --> new
    QOK -->|так| new["new-plan · Новий план — незбережений<br/>на чому стоїть · вердикт · маршрут і карта · що треба · дні · нотатки й відгуки"]

    new -->|"тап по дню"| huts["huts · змінити ночівлю<br/>формула гарантії на кожній хижі"]
    huts --> new
    new --> notes["notes · нотатки з місця / відгуки"]
    new -->|"Скасувати"| GONE(["план зникає · при виході питаємо"])
    new -->|"Зберегти план"| account["account · акаунт тут, не на вході"]
    account --> notify["notify · «щоб попередити за два дні»"]
    notify --> plan

    plan --> HEALTH("«Що ще потрібно від тебе» — інформує, не блокує<br/>забронювати хижу офлайн + номер броні · квиток на автобус · офлайн-пакет · сказати рідним")
    HEALTH -->|алерт: повінь, прогноз| changes["changes · що змінилось і що ще можна зробити"]
    plan --> day["day · День<br/>шматок маршруту · ночівля · транспорт і квитки · умови · погода"]
    plan -->|"Змінити план"| new
    day --> QHOR{"День у межах прогнозу ~10 днів?"}
    QHOR -->|ні| NORM("day · норма замість прогнозу · поза вердиктом")
    plan -->|"у дорозі"| today("day · сьогодні — стан поточного дня")
```


> ⚠️ **Перебудова 2026-09-23.** Головний потік: **пошук → генерація → новий план → збереження → план → день**. `route` став `new-plan` (показується після генерації, незбережений); `night`, `transport` і `today` — зони й стан `day`; `nights`, `lock-in` і `offline-pack` — зони `plan` («Що ще потрібно від тебе»); `field-notes` і `reviews` — `notes`. Назви екранів у потоках нижче вже перейменовані за цією відповідністю; діаграма головного потоку — переписана.
...
1. **Фільтри дали маршрути?** — гілка «ні» це не помилка, а нормальний результат, і в неї **чотири виходи**: інший місяць, сусідній регіон, коротша дистанція і **збирання маршруту**. *З 2026-09-23 збирання — не окремий екран, а зона того самого аркуша над картою*: зібрані маршрути лягають у той самий список поруч з іменованими, з позначкою «зібрано під твій запит». Наш власний тупик став точкою входу в генеративну фічу — і тепер без жодного переходу. Генерація стартує кнопкою, а не на кожну зміну фільтра: це запит до моделі, секунди й гроші.
2. **Ланцюжок ночей складається?** — розвилка генеративного входу. Порожній результат тут — **картка розриву в тому самому списку**, яка **називає, де саме розрив і чому** («між Sitasjaure й Vaisaluokta 38 км — це не денний перехід»), а не каже «нічого не знайдено». Komoot поріже 200 км за цільовою довжиною дня й підкладе generic-житло; ми ріжемо по місцях, де реально можна спати.
...
2б. **Є мережа для генерації?** — генерація це серверний виклик `claude-opus-5`, тому без мережі плану не буде **взагалі**. Кнопка блокується, бриф зберігається, генерація стартує сама при появі мережі. **Стан тепер живе в одному місці — на `new-plan`**, а не на двох різних екранах залежно від персони, як було у візарді.
...
6. **Є конфлікт?** *(замінило `QAGREE`, рішення №8)* — **головна правка всієї ревізії**. Раніше потік вів людину через `day`, `gear`, `day` і потім питав її, чи все сходиться — тобто робив її інтеграційним шаром рівно там, де MAIN JOB обіцяє це зняти. Тепер узгодженість **обчислюється**: зведений вердикт зверху `plan` плюс чотири сигнали в рядку кожного дня, а конфлікт названо конкретно.
7. **Яким входом правимо?** *(рішення №10)* — три входи за типом зміни: контекстна дія в місці наслідку, «Параметри походу» — фільтри й «змінити маршрут» — для дат, регіону й маршруту, вільний текст для складного. Розвилка **«є вже закріплені кроки в `plan`?»** обов'язкова: зміна дат після оформленої броні ламає 7, і людина мусить побачити це **до** застосування, а не після.
...
- **Успіх** — план **сходиться, і це сказав продукт**, а не людина після обходу чотирьох екранів; далі `plan`, `share` або офлайн-пакет із встановленням.
- **Успіх офлайн** — окремий термінал, а не пауза: план, ночівлі **й останній відомий розклад транспорту** читаються з кешу, видно час останньої перевірки. До ревізії офлайн-гілка нікуди не вела, а `day` у неї не входив.
...
⚠️ **У цього потоку є вхід до того, як план існує.** **Превʼю формули гарантії стоїть на `new-plan`, картці маршруту** — по хижах вздовж маршруту, у вигляді, який ще не знає складу групи й членства, і прямо про це каже: «для тебе це виглядатиме інакше — залежить від членства й складу групи».
...
flowchart TD
    ...
    QENTRY -->|з навігації| nights["plan · дні — ланцюжок ночей"]
    day --> night["day · ночівля: гарантія й доступ"]
    nights --> night
    ...
    HANDM -.-> C1REF["plan · Що ще потрібно від тебе"]
...
- **Тупик членства** — замкнена хижа + жодного члена + відмова оформлювати. **Вихід — контекстна дія в точці збою** — заміна ночівлі просто на `day`. Відновлення, що живе на іншому екрані та ще й через вільний текст, змушувало людину переформульовувати те, що продукт уже знає.
...
⚠️ **Три класи.** Job дослівно каже «щоб **ночівлі, квитки й членство** справді оформлені». Тепер `plan` веде три черги з різними дедлайнами й різними системами передачі:
...
| **Квитки** | deep link у чекаут оператора з підставленими параметрами; квиток потім **зберігається в `day`** | розклад і ціна, що росте |
...
```mermaid
flowchart TD
    IN(["План готовий, жодна ніч не закріплена"]) --> lockin["plan · Що ще потрібно від тебе"]
    lockin --> QANY{"Є кроки до виконання?"}
...
**Цикл, а не лінія**: потік повертається в `plan` після кожного кроку, бо DNT вимагає *«complete the booking and payment for one cabin before proceeding to the next»*.
...
flowchart TD
    ...
    QKIND -->|"сезон або об'єкт ночівлі"| night["day · ночівля: гарантія й доступ"]
    QKIND -->|транспорт| transport["day · транспорт і квитки дня"]
    ...
    QNEWSTEP -->|так| lockin["plan · Що ще потрібно від тебе"]
    QNEWSTEP -->|ні| WIN2
    lockin --> WIN2(["УСПІХ · план перебудований, поки альтернатива існувала"])
...
1. **План уже «у дорозі»?** *(нове)* — вхід роздвоєний. До ревізії потік починався тільки з `day`, тобто зміни на етапі підготовки не мали куди приходити, хоча 3 прямо їх називає.
2. **Є мережа?** — перше рішення, а не останнє: offline-first обов'язковий, і `day` мусить бути корисним без мережі. Тепер офлайн має **свій успішний кінець**, а не повернення в ту саму перевірку.
...
Потік доданий після критики: `notes` не проходив через жоден сценарій, а стан `degraded` «розходження офіційних даних із місцевістю» — **підстава існування цього екрана** — не був протрасований ніде.

```mermaid
flowchart TD
    IN(["Сумнів: чи витягну саме цю ділянку"]) --> day["day · День"]
    day --> fieldnotes["notes · Нотатки з місця"]

    fieldnotes --> QANY{"Є нотатки для цієї ділянки?"}
...
flowchart TD
    ...
    QWHY -->|"безпека — рекомендація DNT"| today["day · сьогодні"]
...
- **Успіх безпековий** — маршрут і очікуваний час повернення передані; доступно з `day` і `changes`.
...
6. **Один екран народився з навігації, а не з jobs** (`plan`) і **одна сутність — із критики потоків** (19, Точка сходу). Обидва механізми появи варто тримати видимими: перший — потреба структури, другий — дірка, знайдена перевіркою. Ні той, ні той не є голосом людини, тому обидва перші в черзі на перевірку.
