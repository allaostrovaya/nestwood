...
**Статус**: чинна структура — **пʼять вкладок, 22 екрани, 12 jobs**, матриця трасування закривається в обидві сторони. Розділи: **«Сутності»** (інвентаризація об'єктів), **«Екрани»** (дерево), **«Навігація»** (модель руху, рівні, глибина в тапах), **«Трасування»** (матриця job × екран зі списками сиріт) і **«Рішення»**. Потоки — в окремому файлі [flows.md](./flows.md).
...
**Жорстка межа**: нотатка — це **стан** («міст знесло, брід по пояс»), відгук — це **думка** («красиво, але людно»). Обидва тепер існують, але як **дві різні сутності в двох різних місцях**: нотатка тут (17, на ділянці й обʼєкті), відгук — сутність 22 на `lodging-system` і `day`. Раніше межа трималась тим, що відгуків не існувало; тепер вона тримається роздільністю. Що не змінилось: community ніколи не змішується з офіційним в одному рівні довіри — інакше ми втрачаємо єдину річ, яку продаємо.
...
**Поверхня**: **`new-plan` «Картка маршруту»** — центральний екран продукту, вхід із каталогу `catalogue` — і для іменованого маршруту, і для зібраного під запит; зведено вгорі **`plan`** з дією «змінити маршрут».
...
3. **Оцінка не сортує маршрути у `new-plan`.** Сортування за рейтингом — це і є перетворення планувальника на машину популярності. У `new-plan` маршрути впорядковані за **відповідністю брифу**; рейтинг видно, але він не ранжує.
...
```
Карта
└ Куди можна піти                       ← хаб: рядок пошуку + карта + аркуш
  └ Новий план ⭐                        ← незбережений; стан «генерація» перед ним
    ├ Хижі маршруту → Хижа               ← аркуші: змінити ночівлю
    ├ Нотатки й відгуки                  ← аркуш
    ├ Зберегти похід                     ← аркуш: акаунт
    └ Попереджати про зміни              ← аркуш: дозвіл

Плани
└ Мої походи                            ← хаб вкладки
  └ План ⭐                              ← зверху «Що ще потрібно від тебе»
    ├ День                              ← тримає ніч, транспорт, умови; «сьогодні» — його стан
    ├ Спорядження
...
#### Вкладка 1 · **Карта** — пошук, генерація, новий план

⚠️ **Перебудова 2026-09-23.** Головний флоу тепер іде так: **пошук → генерація → новий план → збереження → план → день**. Картка маршруту, ніч, ланцюжок ночей, дорога туди й назад, останні кроки, офлайн-пакет і «Сьогодні» перестали бути окремими екранами — їхній зміст переїхав у новий план, план і день. Таблиця відповідності — нижче, «Куди поділись екрани».

- **`catalogue` · 0.1. Куди можна піти** — **рядок пошуку як у Booking чи Airbnb** над **картою на весь екран**, список — в аркуші, що тягнеться на три висоти · job MAIN, 1 · P S
  - `зона` **Куди й коли**: країна → регіон · **гнучкі дати** («5 днів у серпні»; точні — пізніше, коли від них залежать броні) · **хто йде** — лічильники «людина» і «людина-член DNT/STF», відкриваються після вибору країни · тип ночівлі (хижі / намет). Поля мають типові значення, результати видно одразу — рядок пошуку не стає формою · job 1, 2, 9 · P S
  - `зона` **Карта**: лінії пресетів і **задані точки старту й фінішу** — станції, зупинки, входи на марковану мережу, хижі (як місця посадки в Uber). Тапом двох точок людина складає **свій маршрут** — це і є генеративний вхід: маршрут, якого ніхто не називав · job MAIN, 1 · P S
    ⚠️ **Тап між точками прилипає до найближчої**, тому зібраний маршрут **ніколи не сходить з маркованої мережі**; пошук іде по **графу хиж**, не по вершинах карти.
  - `зона` **Найкраще в [країна] в [місяць]** — пресети маршрутів із днями · job 1 · P S
  - `зона` **Класика Скандинавії** — курований добір по обох країнах · job 1 · P S
  - `стан` **нічого не сходиться** (напр. лютий — хижі зачинені) → інші дати, класика на літо
- **`new-plan` · 0.2. Новий план** ⭐ — **незбережений** план: «Скасувати» — і він зникне; при виході питаємо · job MAIN, 1, 2, 4, 6, 9, 11 · P S
  *Раніше `route` «Картка маршруту». Тепер це те, що людина бачить після генерації: рішення «чи це для мене» приймається тут, коли ночівлі, квитки й навантаження вже видно.*
  - `стан` **генерація** — анімований екран між пошуком і новим планом; етапи названі джерелами · MAIN, 5
  - `зона` **на чому це стоїть** — країна, дні, хто йде, тип ночівлі; зміна перераховує план · 9, 5
  - `зона` **чи план складається** — вердикт; погода поза 10 днями — сезонна норма, не прогноз · 1, 3
  - `зона` **маршрут і карта** — довжина, набір, найдовший перехід, **навантаження за названим методом**, офіційна градація по ділянках · 1
  - `зона` **що треба для цього походу** — членство (DNT/STF, скільки з групи), сезонна погода, спорядження, вартість · 2, 4, 7
  - `зона` **дні** — компактні рядки; дорога туди й назад — перший і останній рядок; тап по дню → змінити ночівлю (`huts`) · 2, 6
  - `зона` **нотатки й відгуки** → `notes` · 11 + розходження з офіційними даними
  - `зона` **зберегти** → `account` (акаунт і дозвіл на сповіщення з'являються тут, не на вході) · 7
  - `стан` **неможливий маршрут** — розрив у ланцюжку ночей або навантаження не вміщається: **зберегти не можна**, виходи названо («+1 день · інший фініш · намет»)
  - `стан` **помилка** — критичне джерело (стежки) не відповіло: плану немає, здогад не підставляємо
- **`huts` · 2.2. Хижі маршруту** (аркуш) — заміна ночівлі до збереження, формула гарантії на кожній · job 2 · P S · `стан` порожній — на ділянці хиж немає
- **`hut` · 2.3. Хижа** (аркуш) — обʼєкт: тип, ключ, ліжка, сезон, що на місці · job 2, 8 · P S
- **`notes` · 11.1. Нотатки й відгуки** (аркуш) — дві вкладки, **два різні обʼєкти**: нотатка про стан може змінити план, відгук — ніколи · job 11 + розходження даних · P S
- **`account` · 7.3. Зберегти похід** (аркуш) — акаунт саме тут: план із бронями має пережити закритий застосунок · job 7 · P
- **`notify` · 3.3. Попереджати про зміни** (аркуш) — дозвіл, коли він щось означає: «щоб попередити за два дні» · job 3, 6 · P

#### Вкладка 2 · **Плани** — збережені походи

- **`plans` · —. Мої походи** — майбутні / минулі · **[не-job: сховище]** · P
  ⚠️ **Вкладка відкривається самим активним планом**, не списком. Дім контекстний: план, коли похід активний або старт за день-два; карта в решті випадків.
- **`plan` · 0.3. План** ⭐ — збережений план · job MAIN, 1, 3, 4, 7, 9 · P S
  - `зона` **Що ще потрібно від тебе** — зверху: **алерти й задачі, які інформують, але не блокують** · job 3, 6, 7, 10 · P S
    *Забронювати хижу офлайн і вписати номер броні · купити квиток на автобус · завантажити офлайн-пакет · сказати рідним, куди йдеш. Алерт («повінь — у день 2 не встигаєш до Alesjaure») веде в `changes`. Раніше це були окремі `lock-in` «Останні кроки» й `offline-pack`.*
    ⚠️ **Три класи задач**: ночівлі · квитки · членство — різні дедлайни й системи передачі. **Квитки зберігаємо, не продаємо** (EU Package Travel Directive).
  - `зона` **на чому це стоїть** + «Змінити план» → `new-plan` (після першого збереження план редагується) · 9
  - `зона` **маршрут і карта** — карта з шарами (дні, хижі, точки сходу, вода й броди, транспорт, градація) живе тут · 1, 2, 6
  - `зона` **про похід** — членство й ключ, спорядження, погода, вартість · 2, 4, 7
  - `зона` **дні** — компактні рядки зі станом словом; тап → `day`; дорога туди й назад — перший і останній рядок · MAIN
  - `стан` **у дорозі** — зверху сьогоднішній день · `стан` **пройдений** — лишається цілим, з нього рахується темп · `стан` завантаження (перевірка свіжості) · помилка · офлайн · degraded (живої наявності ліжок немає)
- **`day` · 0.4. День** — тримає все своє: **шматок маршруту, ночівлю, транспорт і квитки цього дня, умови, погоду** · job 1, 2, 3, 6 · P S
  *Окремою сторінкою, не акордеоном: сповіщення «день 4 — прогноз погіршився» веде в конкретний день, між днями гортаєш свайпом. Раніше тут жили окремі `night`, `transport` і `today`.*
  - `зона` зараз (у дорозі — скільки лишилось, до котрої прийти) · `зона` перехід (карта дня, профіль, навантаження) · `зона` **ночівля** — формула тип × бронь × час приходу × членство, що зробити на місці · `зона` транспорт і квитки цього дня · `зона` умови на ділянці → `notes` · `зона` погода · `зона` інші дні
  - `стан` **норма замість прогнозу** · `стан` **сьогодні** (у дорозі) · `стан` офлайн · `стан` degraded
- **`gear` · 4.1. Спорядження** · job 4 · P **S** — вага рюкзака входить у розрахунок часу переходу (Tranter)
- **`changes` · 3.1. Що змінилось і що ще можна зробити** · job 3, 6 · P S — скільки часу лишилось на реакцію
- **`share` · 10.1. Сказати, куди йду** · job 10 · P S — довіра близьких і безпековий share, який рекомендує DNT
...
| `lodging-system`, `rules`, `field-notes` | **`route`**, **`field-notes`**, карта в **`plan`** |
...
| **2026-09-23:** `route` Картка маршруту | **`new-plan`** Новий план — показується після генерації, незбережений |
| **2026-09-23:** `night` Ніч, `transport` Дорога туди й назад, `today` Сьогодні | зони й стан **`day`** День |
| **2026-09-23:** `nights`, `lock-in` Останні кроки, `offline-pack` | зони **`plan`**: дні й «Що ще потрібно від тебе» |
| **2026-09-23:** `field-notes`, `reviews` | **`notes`** — один аркуш, два обʼєкти |
| **2026-09-23:** `plan-conflict`, `plan-empty` | стан **`new-plan`** «неможливий маршрут» — до збереження, а не після |
...
|---|---|---|
| MAIN | **`plan`** (вердикт + чотири сигнали) · **`new-plan`** (превʼю узгодженості до генерації) | **↑** — з'явився ранній показ: теза видна на другому тапі, а не на четвертому |
| 1 | **`catalogue`** (фільтр «днів», збирання), **`new-plan`** (профіль, градація, навантаження), `plan` (карта), `day`, `me` | ↑ — плюс обчислене навантаження як механізм, а не лише показ цифр |
| 2 | **`day`** (формула), `plan`, **`new-plan`** (превʼю формули), `lodging-system`, `membership` | ↑ на картці, **↓ у навігації** — див. попередження нижче |
| 3 | `changes`, `day`, `day` (сезонна норма), **10** (точка сходу), пуші | = |
| 4 | `gear`, `my-gear`, **сповіщення «ти спакувався?»** | **↑** — з'явився момент, коли це важливо |
| 5 | `шар` «Звідки це взято» на `plan`/`day`/`day` · джерело+ліцензія на фото, нотатках, відгуках · **`guide`** (джерело статті) · ©Kartverket | = ⚠️ живе шаром, не екраном — інваріант, який перевіряє аудит |
| 6 | `day`, **`new-plan`** (досяжність), `changes`, **10** (точка сходу) | ↑ |
| 7 | **`plans`** (вкладка відкривається в план → «чого бракує» на глибині нуль), **`plan`** (три класи), `plan`, `membership` | ↑ у доступності, **↓ у навігації** |
| 8 | **`guide` Довідник** (пошук + фільтр по країнах), `lodging-system`, `plan` | **↑↑ найбільший виграш** — раніше цей вміст був доступний лише тому, хто вже згенерував план |
| 9 | **зона «на чому це стоїть» на `plan`**, `me` | = — тримається інваріантом замість питань наперед |
| 10 | `share` | = |
| 11 | `notes` (відгуки), `notes` (нотатки), `new-plan` | = ⚠️ **на старті порожній** — відкрите питання |
...
| 3 План походу | `plan` |
| 4 День плану | `day` |
| 5 Ділянка маршруту | всередині `day`; розходження з місцевістю — стан `degraded` на `day`/`notes` |
| 6 Офіційна градація | `шар` у `day` |
| 7 Об'єкт ночівлі | `day`, `plan` |
| 8 Картка гарантії й доступу | `day`, `plan` |
| 9 Прогноз погоди | `day`, `day` |
| 10 Зміна / сигнал | `changes` |
| 11 Спорядження | `gear`, `my-gear` |
| 12 Транспортне плече | `day` |
| 13 Пояснення рішення | `шар` у `day` |
| **14 Джерело даних і його стан** | **персистентний рядок на `plan`/`day`/`day` + `шар` «Звідки це взято»** |
| 15 Локальні правила | `lodging-system` |
| 16 Поділюваний підсумок | E |
| 17 Польова нотатка | `notes` |
| 18 Крок закріплення | `plan` |
| 19 Точка сходу | `changes` — зокрема стан «варіантів немає»; поле дня на `day` |
| **20 Маршрут-кандидат** | **`new-plan` «Картка маршруту»** (вхід із `catalogue` — іменований або зібраний); зведення вгорі `plan` з дією «змінити маршрут» |
| **21 Фотографія** | `catalogue` (регіон), `new-plan` (галерея маршруту), `day` (обʼєкт ночівлі), `notes` (свідчення стану). ⚠️ **Джерела**: Wikimedia Commons, Mapillary (CC BY-SA), Flickr під CC, фото асоціацій за угодою, фото спільноти. **Google Places не беремо** — їхній контент не можна кешувати, тобто фото не потрапило б в офлайн-пакет, плюс це змусило б узяти і їхню карту, а покриття гірських перевалів у Places усе одно немає |
| **22 Оцінка й відгук** | **`notes` окремим списком**, `new-plan` (маршрут), `day` (хижа) — завжди на рівні `спільнота`, візуально відділено від нотаток. ⚠️ **На старті порожній**, і засіяти чесно неможливо — відкрите питання |
| **23 Умова на ділянці** | **`notes`** (стрічка по маршруту), `new-plan` (зведення на картці), `day` (по днях), карта в `plan` (шар). ⚠️ Нотатка про стан **може** впливати на план, оцінка — ніколи. Типи чіпами: `стежка · вода · брід · сніг · місце для намету · маркування` — «місце для намету» третій цінний тип, якого немає в жодному офіційному наборі |
...
- **empty** — немає жодного плану (`plans`), немає нотаток з місця для цієї ділянки (`notes`), **рейсів у ці дати немає (`day`)**, **план не склався в задані параметри (`plan`) — з 2–3 готовими виходами** *(рішення №11, №13)*, **фільтри нічого не дали (`catalogue`) — збирання в тому самому аркуші стоїть першим**, **відгуків ще немає (`notes`)**
...
- **loading** — генерація плану (на `new-plan`, де стоїть дія генерації), перевірка змін (`changes`), **перевірка свіжості при відкритті збереженого плану — один запит на весь план, не на кожен день** (той самий виклик живить `Зміну/сигнал`, тому `day` власного `loading` не має), **стан рейсу при відкритті `day`**
- **error** — джерело даних недоступне; **на `day` — тільки якщо кешу теж немає** (інакше це `offline`/`degraded`)
- **degraded** — «наявність ліжок очікує угоди з DNT/STF» на `day`; розходження офіційних даних із місцевістю на `day`/`notes`. **Обов'язковий** з першого дня, не пізніший edge-case — вимога [README.md](./README.md).
...
- **offline** — на `plan`, `day`, `day`, **`day`**, `day`. **Мусить мати завершення, а не петлю**: «працюй із тим, що є» — це нормальний кінець сценарію, а не очікування мережі
  ⚠️ **На дії генерації offline має протилежний зміст**: генерація — серверний виклик, тож без мережі плану не буде взагалі. «Скласти мій план» блокується, бриф (фільтри + маршрут) зберігається, генерація стартує сама при появі мережі. Візарда більше немає, тому стан живе **на `new-plan`, поруч із дією генерації** — в одному місці, а не на двох різних екранах залежно від персони. Це єдина точка продукту, де офлайн не має продуктового результату, і зводити її з «працюй із тим, що є» в один стан не можна
- **у дорозі** — стан `Плану`, який робить доступним **`day`** (`changes` від цього стану не залежить)
...
- **Рейтинги й відгуки є** — сутність 22, із чотирма жорсткими межами. Головна з них — рейтинг **не сортує маршрути**: у `new-plan` і `guide` порядок задає відповідність брифу, а не популярність
...
Другою колонкою рахую **2 / Top Job #1** (екран **`day`**) — не як альтернативний main job, а тому що це найгустіше покритий job набору й наш диференціатор: якщо він глибокий, диференціатор не працює незалежно від того, як швидко відкривається план.
...
|---|---|---|---|
| **Карта** | 1, 2, 4, 6, 9, 11, MAIN | `catalogue` (пошук + карта + аркуш, свій маршрут тапами) → генерація → `new-plan` → `huts`, `notes`, `account` | Вхід і каталог — і сам є картою. Тут людина отримує зміст **на нульовому тапі**, а не форму. Фільтри тут — це бриф, тому вкладка одночасно є найширшим інструментом вводу в продукті |
| **Плани** | **MAIN**, 3, 4, 6, 7, 10 | `plans` → `plan` → `day`, `gear`, `changes`, `share` | Дім, коли похід є. **Відкривається самим активним планом**, не списком, тому 7 («чого бракує») стоїть на глибині нуль. Усе про похід живе **всередині** плану, а не поруч із ним — це теза продукту, виражена структурою |
...
- **Вкладки «Квитки»** — квиток без походу не має сенсу. Файли й статус живуть у `day`, всередині плану.
...
- **«Транспорт»** — навпаки: доказова база 6 найсильніша в усьому дослідженні, і все одно вкладкою не стає. **Сила доказу ≠ частота доступу.** Вимога інша — його має бути **неможливо пропустити**: тому досяжність стоїть на `new-plan`, а плече інлайн першим і останнім елементом списку днів на `plan`.
...
| **Повторний, новий похід** (акаунт заповнений — хто йде вже підставлено) | Карта → пресет → генерація → новий план → «Зберегти план» | **2** | 3 |
| **Холодний старт** | Карта → «Хто йде» в рядку пошуку → пресет → генерація → новий план → «Зберегти план» | **3** | 4–5 |
| Свій маршрут | Карта → тап старту → тап фінішу → «Скласти план» → новий план → «Зберегти план» | **4** | *шляху не існувало* |

**Стеля була чотири, стала три** — і зникла причина, через яку четвертий тап не скорочувався. Раніше він існував, щоб не генерувати, не знаючи членства (змінна №4 формули гарантії). Тепер членство питається **в рядку пошуку, у лічильнику «хто йде»** (з 2026-09-23 — як у Booking чи Airbnb, раніше це був рядок на картці маршруту), тобто до генерації воно вже відоме, а окремого екрана під це немає. **Цінність видно до збереження**: ночівлі, квитки й навантаження — у новому плані, одразу після генерації.

**До 2 / Top Job #1 (`day` — гарантія й доступ на ніч), як контрольна метрика диференціатора:**
...
| Рівень | Що це | Склад |
|---|---|---|
| **Глобальне** — видно завжди | Пункти, у яких людина живе, і зобов'язання, що діють постійно | **План** · **Ночівлі** · **Закріпити** · **Моє** · **Сьогодні** (у стані «у дорозі») · **персистентний рядок джерел** на `plan`/`day`/`day` (не пункт навігації, а обов'язковий елемент хрому — рішення №8) · індикатор `offline` |
| **Контекстне** — з'являється в потоці | Прив'язане до об'єкта чи моменту, поза ним не має сенсу | `day` (день зі списку) · **`gear` (два входи: з `plan` і з `day` — обидва рівноправні)** · **`day` (інлайн на початку й у кінці списку днів)** · `notes` (нотатки на ділянці / об'єкті) · `lodging-system` (з `day`, плюс постійний вхід із шапки «Ночівель») · `membership` (з `plan` і з `day`, коли членства бракує) · **`changes` (коли є незакрита зміна — з `plan` і `plan`)** · **безпековий share із `day` і `changes`** · `шар` градації і `шар` «чому саме так» на `day` · `шар` розмовного уточнення на `plan` · стан `degraded` там, де він застосовний |
...
3. **«Сьогодні» перехоплює дім у стані «у дорозі»**, але лишається станом вкладки «Плани» — `day` і `plan` різні за вмістом і за вимогами (офлайн, економія батареї), а не два стани одного макета. **Умовність стосується `day`, не `changes`**: незакрита зміна відкриває `changes` і на етапі планування.
...
7. **Офлайн не змінює навігацію** — змінює вміст. `plan`, `day`, `day`, **`day`** і `day` лишаються доступними; недоступне позначається, а не ховається (принцип «деградувати чесно»). **Виняток один — дія генерації**: там офлайн не «змінює вміст», а зупиняє генерацію, бо плану без сервера не існує.
...
flowchart LR
    ...
    CAT -->|пресет або свій маршрут| GEN("генерація")
    GEN --> NEW["new-plan · новий план ⭐<br/>незбережений"]
    NEW --> HUTS["huts · змінити ночівлю"]
    NEW --> NOTES["notes · нотатки й відгуки"]
    NEW -->|зберегти| ACC["account"]
    ACC --> PLAN
    PLAN --> DAY["day · тримає ніч, транспорт, умови"]
    PLAN --> GEAR["gear"]
    PLAN --> CH["changes"]
    PLAN -.поділитись.-> SH["share"]
    DAY --> LS["lodging-system"]
    PLAN -->|у дорозі| DAY
    DAY --> SAFE
...
Дванадцять jobs × 22 екрани. ✓ ставиться, тільки якщо екран реально бере участь у закритті job'а — не «дотичний» і не «міг би».

### Матриця A · Карта (пошук і новий план)

| Job | `catalogue` Карта | `new-plan` Новий план | `notes` Нотатки й відгуки |
|---|:-:|:-:|:-:|
| **MAIN** | ✓ | ✓ |  |
| 1 | ✓ | ✓ | ✓ |
| 2 | ✓ | ✓ |  |
...
*2026-09-23, дві перебудови того самого дня: спершу `map` і `assemble` злиті (збирання → свій маршрут на карті `catalogue`; карта маршруту → зона `plan`), потім `new-plan` став `new-plan`, `notes` + `notes` — `notes`. У матриці B `plan` вбирає `plan`, `plan` і `plan`, а `day` — `day`, `day` і `day`: ✓ злитого стовпця — об'єднання ✓ колишніх.*

### Матриця B · Плани

| Job | `plans` | `plan` План | `day` День | `gear` Спор. | `changes` Змінилось | `share` Сказати |
|---|:-:|:-:|:-:|:-:|:-:|:-:|
| **MAIN** |  | ✓ |  |  |  |  |
| 1 |  | ✓ | ✓ |  |  |  |
| 2 |  | ✓ | ✓ |  |  |  |
...
| Порожніх стовпців | **3** — `plans`, `plan`, `settings` | 1 |
| Jobs з покриттям рівно на одному екрані | **1** — 10 (`share`). *MAIN вийшов зі списку: `new-plan` дав йому другу поверхню* | 2 |
...
**Порожніх рядків — жодного.** **Порожніх стовпців — три**, у кожного названа не-job-підстава: сховище (`plans`), констрейнт (`plan`), поведінка застосунку (`settings`). Плюс одна вкладка, половина якої тримається гіпотезою H6 — і це записано в `jtbd.md`, а не замасковано під job.
...
| **Екран, недосяжний зі свого ж тупика** (`changes` із `plan`) | Досяжність не є вимірюваною величиною матриці — вона знає, що `changes` закриває 3, і не знає, що `changes` було замкнено станом | Правило навігації №3, виправлене: умовний лише `day` |
...
| **Екрани з job'ом, але без жодного сценарію** (`notes`, E) | `✓` у матриці означає «має підставу», не «перевірений» | Потоки 11 і 10, додані в [flows.md](./flows.md). Без потоку свідомо лишаються тільки `me`–`membership` — це керування збереженим, не job-шлях |
...
**Намальовано все: 22 екрани і 20 станів = 42 сторінки** *(2026-09-23: було 31 + 32 = 63; дві перебудови того самого дня — злиття карти й збирання з каталогом, потім головний флоу «пошук → генерація → новий план → план → день»)*. Сірих вузлів у дереві не лишилось, заглушок у макетах теж — кожен названий вихід веде на реальний екран.

**Що стоїть на `new-plan`, і чого не можна вирізати:**
...
**Три поверхні, яких раніше не існувало:** `guide` (job 8 нарешті доступний без плану), `safety` + `first-aid` (сутність 19 отримала постійну поверхню; SOS і перша допомога тримаються гіпотезою H6), збирання маршруту, якого в каталозі немає (з 2026-09-23 — зона `catalogue`, а не окремий екран).

**Правило відбору лишається чинним**: екран без job'а або без місця в потоці не малюється — з трьома винятками, у яких названа не-job-підстава (`plans` — сховище, `plan` — констрейнт, `settings` — поведінка застосунку).
...
|---|---|---|
| 1 | **7 — job, і під нього guidance-шар без власних транзакцій** | Сутність 18 (`Крок закріплення`) і екран `plan`. Передача в офіційну систему — вихід із продукту, не наш екран |
| 2 | **Планування + тонкий шар «у дорозі»** | Механізм запасу часу живе в продукті, а не на слайді: `day` і `changes` |
...
| 10 | **Три входи правки плану** | Контекстна дія в місці наслідку · параметри походу (фільтри й «змінити маршрут») · розмовний шар для складного. Зміна параметрів попереджає, що зламає в `plan` |
...
| 19 | **Вибір маршруту — рішення людини, а не побічний ефект генерації** | Сутність 20 і екран `new-plan`. Без обраного маршруту «чотири джерела узгоджені» стосується походу, якого ніхто не обирав |
...
| 21 | **План має три фази, і це стани одного обʼєкта** | `plan` (попереду) · `plan-intrip` (у дорозі) · `plan-past` (пройдений). Фаза визначається датами, а не вибором людини. «Сьогодні» не окрема поверхня й не шоста вкладка: у фазі «у дорозі» це **перша картка дашборда** з переходом на `day`. У фазі «пройдений» зникають «закріпити» й «що змінилось» як завдання, а припущення заморожуються — інакше запис перестає відповідати тому, що сталось |
| 23 | **Хижа — самостійний обʼєкт, ніч — ні** | `hut` існує без дат: тип, ключ, ліжка, сезон, сусіди й відстані. `day` = хижа × дата × бронь × час приходу × членство. `huts` — один пошук на два контексти: з чого складається ланцюг при плануванні і що ще досяжне, коли ніч зривається в дорозі. Розрив у ланцюзі (`huts-empty`) — відповідь, а не порожній результат: «38 км до наступного даху — це не денний перехід» |
...
| **8. Окремий екран «під себе» виявився зайвим** | Фільтри вже є брифом, акаунт дає решту. Лишаються два поля (склад групи, членство) — **рядок на картці, не екран**. Точні дати переїхали в `plan`, де від них залежать дедлайни |
...
**Дві речі, які не можна вирізати при малюванні** (позначені ⚠️ у дереві): **превʼю формули гарантії на `new-plan`** — доки плану немає, це єдине місце, де 2 видно взагалі; і **шар джерела на кожній поверхні, що щось стверджує** — він живе в `_conventions.md` як інваріант аудиту, бо шари губляться при перемалюванні.
