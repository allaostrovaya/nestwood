...
**Статус**: чинна структура — **пʼять вкладок, 20 екранів, 12 jobs**, матриця трасування закривається в обидві сторони. Розділи: **«Сутності»** (інвентаризація об'єктів), **«Екрани»** (дерево), **«Навігація»** (модель руху, рівні, глибина в тапах), **«Трасування»** (матриця job × екран зі списками сиріт) і **«Рішення»**. Потоки — в окремому файлі [flows.md](./flows.md).
...
**Поверхня**: **10 «Безпека»** — точка сходу нарешті отримує постійну поверхню, а не лише згадку в стані; плюс `day` (перелік «які альтернативи ще доступні і які вже ні») і `День` на `day`. **Власного екрана-каталогу виходів не отримує** — інакше з'явився б каталог без job'а під ним.
...
```
Карта
└ Куди можна піти                       ← пошук «Куди · Коли · Хто» + карта + пресети з фото
  └ Новий план ⭐                        ← незбережений; «генерація» — скелетон перед ним
    ├ Інша хижа → Хижа                   ← аркуші
    └ Нотатки й відгуки                  ← аркуш, «Написати нотатку»

Плани
└ Мої походи                            ← у поході відкриває сьогоднішній день
  └ План ⭐                              ← загальні алерти, під ними дні зі своїми
    ├ День                              ← «сьогодні» — його стан
    │ └ Забронювати                     ← аркуш поза основним флоу: ночі й квитки
    ├ Спорядження
    └ Сказати, куди йду

Довідник                                Безпека              Профіль
└ Довідник            ← хаб             └ Безпека  ← хаб     └ Я  ← хаб
  └ Стаття                                └ Перша допомога     ├ Вхід (аркуш)
                                                               ├ Моє спорядження
                                                               ├ Членство й ключ
                                                               └ Налаштування   ← шестерня в шапці «Я»
                                                                 └ Вхід (аркуш)
```
...
⚠️ **Дві перебудови 2026-09-23.** Флоу: **пошук → генерація → новий план → план → день** (вхід поза основним флоу — вважаємо, що акаунт уже є). Друга перебудова прибрала внутрішні тексти з усіх екранів, звела кожну задачу до однієї сутності й скоротила набір до 33 сторінок. Беклог і рішення по кожному пункту — `_next.md`.

- **`catalogue` · 0.1. Куди можна піти** — **один компонент пошуку «Куди · Коли · Хто»** (як Airbnb / Booking) над картою на весь екран · job MAIN, 1 · P S
  - **Куди**: регіон, пресет або **свій маршрут «звідки → куди»** — точки задані (станції, входи на марковану мережу, хижі), тап прилипає до найближчої, тож маршрут ніколи не сходить з мережі; до генерації на карті видно лінію, км і дні
  - **Коли**: **точні дати** — дедлайни броні відомі одразу · **Хто**: кількість людей і скільки з них члени (учасників окремо не заводимо)
  - `зона` пресети **з фото, привʼязані до точок на карті** — «Найкраще в [країна] в [місяць]» і «Класика Скандинавії»; фото пресета стає заглавним фото плану
  - `стан` нічого не сходиться (лютий — хижі зачинені) → інші дати
- **`new-plan` · 0.2. Новий план** ⭐ — незбережений: «Скасувати» — зникає · job MAIN, 1, 2, 4, 6, 9, 11 · P S
  - заглавне фото з точки старту · під ним один рядок «дати · хто» → змінити
  - стан плану: коли все сходиться — одна плашка «Усе сходиться»; блок з'являється тільки коли щось не так
  - маршрут (превʼю карти, км, набір, дні, найдовший день, градація) · що треба (членство, погода, спорядження, вартість) · дні — компактні рядки, статус тільки для винятків; тап → змінити ночівлю · нотатки й відгуки · «Зберегти план» → одразу `plan`
  - `стан` **генерація** — скелетон нового плану з прогресом, без тексту · `стан` **неможливий маршрут** — зберегти не можна, виходи названо · `стан` помилка
- **`huts` · 2.2. Інша хижа** (аркуш) — компактні картки хиж поруч · `стан` порожній
- **`hut` · 2.3. Хижа** (аркуш) — повна картка ночівлі: тип, бронь, ліжка, ключ, сезон, на місці, нотатки
- **`notes` · 11.1. Нотатки й відгуки** (аркуш) — два обʼєкти; **«Написати нотатку»** — форма з типом чипами · `стан` порожній

#### Вкладка 2 · **Плани** — збережені походи

- **`plans` · —. Мої походи** — компактні картки з фото · **[не-job: сховище]** · P. **У поході вкладка відкриває сьогоднішній день.**
- **`plan` · 0.3. План** ⭐ · job MAIN, 1, 3, 4, 7, 9 · P S
  - `зона` огляд: заглавне фото, рядок «дати · хто», «офлайн ✓» (пакет качається сам по Wi-Fi), «Сказати, куди йду»; **«Змінити план» попереджає й скидає** позначки броней і квитків
  - `зона` **Потребує уваги** — тільки загальні алерти походу; кожен веде туди, де його виправляють (окремого екрана «Що змінилось» немає)
  - `зона` маршрут — превʼю карти, шари в повноекранній карті · `зона` про похід · `зона` **дні — кожен зі своїми алертами**: «Sälka не заброньована» на дні 4, «квиток не куплено» на дорозі назад
  - `стан` пройдений · помилка й офлайн — плашкою зверху
- **`day` · 0.4. День** — перехід (стрілки між днями й свайп), ночівля, на ділянці, погода; **розділ без змісту не показується** · job 1, 2, 3, 6
  - `стан` **сьогодні** — один екран без скролу, **SOS постійно**, «Написати нотатку з місця» · `стан` норма замість прогнозу · `стан` офлайн
- **`booking` · 7.1. Забронювати** (аркуш, **поза основним флоу**) — що бронюємо й дедлайн, порядок ланцюжка, передача в систему STF/DNT чи перевізника, номер броні й квиток (фото/PDF). Той самий аркуш для ночей і для квитків · job 7, 6
- **`gear` · 4.1. Спорядження** — чеклист пакування, причина дрібно біля речі
- **`share` · 10.1. Сказати, куди йду** — одне повідомлення з превʼю: надіслати · посилання · PDF
...
- **`lodging-system` · 2.4. Стаття** *(2026-09-23: шаблон статті; «Правила й fjellvett» злиті сюди)* — колишнє «Як працює ця система ночівлі» — правила, неписані норми, оригінал + переклад · job 2, 8 · **S**
...
- **`account` · 7.3. Вхід** (аркуш) — Apple · Google · пошта, далі системний запит на сповіщення. **Поза основним флоу** (2026-09-23): у демо акаунт уже є, тому «Зберегти план» веде одразу в план; вхід відкривається з налаштувань («Вийти» → аркуш входу) · job 7 · P
- **`settings` · —. Налаштування** *(2026-09-24: відкриваються **шестернею в шапці «Я»**, як у Strava, Komoot, AllTrails; першою секцією — **Акаунт**: пошта, спосіб входу, «Вийти», «Видалити акаунт», якого вимагає App Store)* — похідний режим, геолокація, сповіщення · **[не-job: поведінка застосунку]** · P S
...
| **2026-09-23 (друга):** `changes` і три стани | алерт веде туди, де зміну виправляють — день, ночівля, квиток |
| **2026-09-23 (друга):** `notify` | системний запит одразу після входу |
| **2026-09-23 (друга):** `rules` | статті довідника (`lodging-system` — шаблон статті) |
| **2026-09-23 (друга):** `plan-intrip`, `plan-loading`, `plan-degraded`, `day-degraded` | у поході вкладка відкриває сьогоднішній день; перевірка свіжості — фоном; «немає живої наявності» — не стан, а рядок на ночі |
| **2026-09-23 (друга):** — | **`booking`** Забронювати — новий аркуш поза основним флоу |
...
| 3 | `day`, `day`, `day` (сезонна норма), **10** (точка сходу), пуші | = |
...
| 6 | `day`, **`new-plan`** (досяжність), `day`, **10** (точка сходу) | ↑ |
...
| 19 Точка сходу | `day` — зокрема стан «варіантів немає»; поле дня на `day` |
...
- **loading** — генерація плану (на `new-plan`, де стоїть дія генерації), перевірка змін (`day`), **перевірка свіжості при відкритті збереженого плану — один запит на весь план, не на кожен день** (той самий виклик живить `Зміну/сигнал`, тому `day` власного `loading` не має), **стан рейсу при відкритті `day`**
...
- **у дорозі** — стан `Плану`, який робить доступним **`day`** (`day` від цього стану не залежить)
- **варіантів немає** — стан `day`. Найнебезпечніша точка продукту: зміна сталась, а запас часу на альтернативу вичерпано. Наповнення фіксоване й проєктується явно — **найближча досяжна `Точка сходу` (сутність 19), стан покриття зв'язку, передати маршрут і час повернення близьким**. Чого в ньому нема й не буде: виклику допомоги — це не наша функція (межа сутності 19)
...
|---|---|---|---|
| **Карта** | 1, 2, 4, 6, 9, 11, MAIN | `catalogue` (пошук + карта + аркуш, свій маршрут тапами) → генерація → `new-plan` → `huts`, `notes` → «Зберегти план» → `plan` | Вхід і каталог — і сам є картою. Тут людина отримує зміст **на нульовому тапі**, а не форму. Фільтри тут — це бриф, тому вкладка одночасно є найширшим інструментом вводу в продукті |
| **Плани** | **MAIN**, 3, 4, 6, 7, 10 | `plans` → `plan` → `day`, `gear`, `day`, `share` | Дім, коли похід є. **Відкривається самим активним планом**, не списком, тому 7 («чого бракує») стоїть на глибині нуль. Усе про похід живе **всередині** плану, а не поруч із ним — це теза продукту, виражена структурою |
| **Довідник** | **8** | `guide` → `lodging-system`, `lodging-system` | Раніше цей вміст лежав усередині плану, тобто був доступний **лише тому, хто вже згенерував план** — а 8 це job приїжджого, персони, центральної в пітчі. Плюс це єдиний вміст, якого немає в жодному API, і DNT сам назвав його своєю нерозв'язаною задачею |
...
| **Контекстне** — з'являється в потоці | Прив'язане до об'єкта чи моменту, поза ним не має сенсу | `day` (день зі списку) · **`gear` (два входи: з `plan` і з `day` — обидва рівноправні)** · **`day` (інлайн на початку й у кінці списку днів)** · `notes` (нотатки на ділянці / об'єкті) · `lodging-system` (з `day`, плюс постійний вхід із шапки «Ночівель») · `membership` (з `plan` і з `day`, коли членства бракує) · **`day` (коли є незакрита зміна — з `plan` і `plan`)** · **безпековий share із `day` і `day`** · `шар` градації і `шар` «чому саме так» на `day` · `шар` розмовного уточнення на `plan` · стан `degraded` там, де він застосовний |
...
3. **«Сьогодні» перехоплює дім у стані «у дорозі»**, але лишається станом вкладки «Плани» — `day` і `plan` різні за вмістом і за вимогами (офлайн, економія батареї), а не два стани одного макета. **Умовність стосується `day`, не `day`**: незакрита зміна відкриває `day` і на етапі планування.
...
flowchart LR
    ...
    NEW -->|зберегти| PLAN
    PROF -->|шестерня| SET["settings"]
    SET --> ACC["account · вхід"]
    PLAN --> DAY["day · тримає ніч, транспорт, умови"]
    PLAN --> GEAR["gear"]
    PLAN --> BK["booking · забронювати"]
    PLAN -.поділитись.-> SH["share"]
    DAY --> LS["lodging-system"]
    PLAN -->|у поході вкладка відкриває сьогодні| DAY
    DAY --> SAFE
...
Дванадцять jobs × 20 екранів. ✓ ставиться, тільки якщо екран реально бере участь у закритті job'а — не «дотичний» і не «міг би».
...
| Job | `plans` | `plan` План | `day` День | `booking` Забронювати | `gear` Спор. | `share` Сказати |
|---|:-:|:-:|:-:|:-:|:-:|:-:|
| **MAIN** |  | ✓ |  |  |  |  |
| 1 |  | ✓ | ✓ |  |  |  |
| 2 |  | ✓ | ✓ |  |  |  |
...
| Job | `guide` Довідник | `lodging-system` Стаття | 10 Безпека | 11 Перша доп. | `me` Про мене | `my-gear` Спорядж. | `membership` Членство | `settings` Налашт. |
|---|:-:|:-:|:-:|:-:|:-:|:-:|:-:|:-:|
| **MAIN** |  |  |  |  |  |  |  |  |
| 1 |  |  |  |  | ✓ |  |  |  |
| 2 |  | ✓ |  |  | ✓ |  | ✓ |  |
...
| **Екран, недосяжний зі свого ж тупика** (`day` із `plan`) | Досяжність не є вимірюваною величиною матриці — вона знає, що `day` закриває 3, і не знає, що `day` було замкнено станом | Правило навігації №3, виправлене: умовний лише `day` |
...
**Намальовано все: 20 екранів і 13 станів = 33 сторінки** *(2026-09-23: було 31 + 32 = 63; дві перебудови того самого дня — злиття карти й збирання з каталогом, потім головний флоу «пошук → генерація → новий план → план → день»)*. Сірих вузлів у дереві не лишилось, заглушок у макетах теж — кожен названий вихід веде на реальний екран.
...
| 2 | **Планування + тонкий шар «у дорозі»** | Механізм запасу часу живе в продукті, а не на слайді: `day` і `day` |
