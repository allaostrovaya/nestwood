...
#### Вкладка 1 · **Карта** — вхід і каталог

- **`catalogue`. Карта** — карта Скандинавії + список маршрутів, перемикач «список / карта», фільтри чіпами · job MAIN, 1 · P S
  *Порядок — за **сезонною доречністю**, не за близькістю: багатоденка ніколи не «поруч» із геопозицією, близькість — логіка одноденних прогулянок AllTrails і для нашого об'єкта безглузде сортування. Геопозиція працює як факт на картці («ти в Осло, вхід у гори за 2 год потягом»), не як порядок.*
  ...
  - `зона` куровані регіони й добірки — «коли туди має сенс» · job 1 · P **S** *(колишній `guide`; сюди ж поїхав контент «про місця» замість окремої стрічки статей)*
  - `стан` **нічого не сходиться** → вихід у `assemble` «зібрати маршрут» *(четвертий вихід із колишнього тупика)*
- **`route`. Картка маршруту** ⭐ — центральний екран продукту · job MAIN, 1, 2, 6, 8, 11 · P S *(колишній `lodging-system`, сутність 20)*
...
- **`field-notes`. Умови на ділянках · нотатки з місця** — стан стежки, маркування, вода, броди, сніг · **розходження офіційних даних із місцевістю** (головна підстава) + 11, 1 · P S *(колишній `field-notes`, сутності 17 і 23)*
...
- **`reviews`. Відгуки** — окремим списком, ніколи не змішані з нотатками · job 11 · P S *(сутність 22)*
...
- **`map`. Повноекранна карта** — шари: маршрут, хижі за типом, точки сходу, транспорт, вода й броди · job 1, 2, 6 · P S *(колишній `rules`)*
  *Поверхня, не розділ. Входи: `catalogue`, `route`, план, день, «Сьогодні».*
- **`assemble`. Зібрати маршрут** — скільки днів, звідки стартую, чого уникати · job MAIN, 1 · P S
...
- **`plans`. Список планів** — активний / майбутні / минулі, **бейдж «скільки закріпити»** · **[не-job: сховище]** · P
...
- **`plan`. План по днях** ⭐ — вердикт узгодженості + чотири сигнали в рядку кожного дня · job MAIN · P S *(колишній `plan`)*
...
- **`day`. День** — перехід, погода, що критично саме сьогодні · job 1, 3, 4 · P S *(колишній `day`)*
...
- **`night`. Ніч: гарантія й доступ** — формула тип × бронь × час приходу × членство · job 2, 8 · P S *(колишній `night`)*
  *Тут же стан «наявність очікує угоди з DNT» і позначка мок-даних **на рядку наявності ліжок**.*
- **`nights`. Ночівлі — ланцюжок ночей** · job 2, 7 · P S *(колишній `nights`)*
...
- **`gear`. Спорядження — чеклист** · job 4 · P **S** *(колишній `gear`)*
...
- **`transport`. Транспорт** — до старту й від фінішу, **зі збереженими квитками** · job 6, 7 · P S *(колишній `transport`)*
...
- **`changes`. Що змінилось і що ще можна зробити** · job 3, 6 · P S *(колишній `changes`)*
  *Центральне поле — скільки часу лишилось на реакцію. Не привʼязаний до стану «у дорозі».*
- **`lock-in`. Що лишилось закріпити** · job 7 · **P** *(колишній `lock-in`)*
...
- **`share`. Підсумок для передачі** — два регістри: довіра близьких (10) і безпековий share, який рекомендує сам DNT · job 10 · P S *(колишній E)*
  - `дія` PDF / друк — паперова копія на випадок мертвого телефона
- **`today`. Сьогодні** — поточний перехід, ночівля на цю ніч, офлайн, **SOS постійно на екрані** · job 3, 6 · P S *(колишній `today`)*
...
- **`guide`. Довідник** — пошук + фільтр по країнах · job 8 · **S** · P (низько)
...
- **`lodging-system`. Як працює ця система ночівлі** — правила, неписані норми, оригінал + переклад · job 2, 8 · **S** *(колишній `lodging-system`; один екран, **два входи** — з `night` і з `guide`)*
...
- **`rules`. Правила й fjellvett** · 8 + **інституційна підстава** · **S**

#### Вкладка 4 · **Безпека** — не влізти в біду і що робити, якщо вліз

- **10. Безпека** — SOS, 112/113, мої координати, **найближча точка сходу**, лавинна небезпека · **3, 6** (точка сходу) + **H6** (SOS) · P S
...
- **11. Перша допомога** — офлайн · **H6** · P S
...
- **`me`. Про мене** — форма, вага, темп, членства; **імпорт зі Strava/Garmin** як фактичне джерело форми замість самооцінки · job 1, 2, 9 · P S *(колишній `membership`)*
...
- **`my-gear`. Моє спорядження** — інвентар · job 4 · P **S**
- **`membership`. Членство й ключ** · job 2, 7 · P **S** *(колишній `membership`; один екран, **два входи** — з `me` і з `lock-in`)*
...
## Замовлення на макети

**Намальовано — 51 сторінка.** Головний потік цілком: `catalogue` · **`route`** ⭐ · `plan` ⭐ · `day` · `night` · `transport`, плюс усе, що вижило зі старої структури, плюс три нові поверхні: **`guide`** (job 8, поверхня приїжджого), **`safety`** і **`first-aid`** (job 3/6 у частині точки сходу, решта — гіпотеза H6).

**Ще не намальовано пʼять**: `reviews` · `assemble` · `offline-pack` · `rules` · `settings`. У дереві вони сірим, посилань на них немає — щоб не робити 404-тупик.

**Що вже стоїть на `route`, і чого не можна вирізати:**
...
- **три блоки складності**, кожен зі своєю підставою: факти й **навантаження за названим методом** (Naismith + Langmuir, Tranter додає вагу рюкзака) · **офіційна градація по ділянках зі станом «не градуйовано»**, який тут типовий випадок (3 ділянки з 7) · умови на ділянках. Ніколи не одне число;
- **досяжність старту й фінішу** — критерій придатності маршруту, а не деталь плану;
- **чесна неповнота** з двома полями вводу (склад групи, членство) — обидва змінюють відповідь, а не сортування.

**Що стоїть на `plan`:** зона **«на чому це стоїть»** — заявлені припущення, які правляться на місці й перераховують вердикт. Це другий інваріант із `_conventions.md`.

**Стани** беруться з розділу «Стани, а не екрани» вище. Правило відбору: екран без job'а або без місця в потоці не малюється — з трьома винятками, у яких названа не-job-підстава (`plans` — сховище, `offline-pack` — констрейнт, `settings` — поведінка застосунку).
