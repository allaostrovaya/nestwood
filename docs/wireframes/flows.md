...
```mermaid
flowchart TD
    START(["Запуск застосунку"]) --> QACT{"Похід триває або скоро старт?"}
    QACT -->|"триває"| today["day · сьогодні<br/>один екран без скролу · SOS постійно"]
    QACT -->|"скоро старт"| plan
    QACT -->|ні| catalogue["catalogue · Куди можна піти<br/>пошук «Куди · Коли · Хто» · точні дати · пресети з фото"]
    catalogue --> QFIT{"Під ці дати щось є?"}
    QFIT -->|ні| NOFIT("empty · хижі зачинені · інші дати")
    NOFIT -.-> catalogue
    QFIT -->|"пресет або свій маршрут<br/>(лінія, км і дні видно до генерації)"| QGEN{"Є мережа?"}
    QGEN -->|ні| ENDGEN(["плану без мережі немає"])
    QGEN -->|так| GEN("new-plan · генерація — скелетон і прогрес")
    GEN -->|"критичне джерело"| ERR("new-plan · помилка · спробувати ще раз")
    GEN -->|"не складається"| CONF("new-plan · неможливий маршрут<br/>зберегти не можна · виходи названо")
    GEN --> new["new-plan · Новий план — незбережений<br/>фото · дати й хто · «Усе сходиться» · маршрут · що треба · дні"]
    CONF --> new
    new -->|"тап по дню"| huts["huts · інша хижа"]
    huts --> new
    new -->|"Скасувати"| GONE(["план зникає"])
    new -->|"Зберегти план"| account["account · вхід + системний запит на сповіщення"]
    account --> plan["plan · План<br/>загальні алерти → дні зі своїми алертами"]
    plan -->|"алерт веде туди, де виправити"| day["day · День"]
    plan -->|"задача на дні або дорозі"| booking["booking · забронювати — поза основним флоу<br/>порядок · дедлайн · передача в STF · номер броні"]
    plan -->|"Змінити план — попередити й скинути"| new
    day --> QHOR{"Понад 10 днів до дня?"}
    QHOR -->|так| NORM("day · сезонна норма")
    today -->|"щось не як на карті"| notes["notes · написати нотатку з місця"]
```


> ⚠️ **Перебудова 2026-09-23 (дві за день).** Головний потік: **пошук → генерація → новий план → вхід → план → день**. `route` став `new-plan`; `night`, `transport`, `today` — зони й стан `day`; `nights`, `lock-in`, `offline-pack` — зони `plan`; `field-notes`, `reviews` — `notes`. Друга перебудова: `changes` зник — кожен алерт веде туди, де зміну виправляють; `notify` — системний запит після входу; бронювання й квитки — аркуш `booking` поза основним флоу; у поході вкладка «Плани» відкриває сьогоднішній день. Назви екранів у потоках нижче перейменовані за цією відповідністю.

**Рішення в потоці**

0. **Є активний або близький похід?** — визначає, який дім людина бачить: **Плани**, якщо похід активний або старт за день-два, **Карта** в решті випадків. Вхід у план іде **через перевірку свіжості**: один запит на весь план, а не на кожен день. Це той самий виклик, що живить `Зміну/сигнал`, тобто вхід у `day` береться звідси, а не з окремого механізму.
...
flowchart TD
    ...
    DEADHARD -.-> D2REF["day · алерт веде туди, де виправити"]
...
- **Тупик жорсткий** — транспорт або сезон. Вихід у **`day`**, і він тепер досяжний: до ревізії `day` був прив'язаний до стану «у дорозі», тобто вихід із тупика вів у замкнені двері.
...
flowchart TD
    ...
    ERR --> changes["day · алерт веде туди, де виправити"]
...
**Стани**: `offline` (зі своїм кінцем) · `loading` (перевірка й перебудова) · `error` (джерело недоступне — і саме воно **є** зміною плану, тому веде в `day`, а не в глухий кут) · `варіантів немає` (новий стан `day`) · стан сигналу «зігноровано».
...
- **Успіх безпековий** — маршрут і очікуваний час повернення передані; доступно з `day` і `day`.
