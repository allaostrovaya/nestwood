...
**Головний потік — шість екранів:** `catalogue` · **`route`** ⭐ · `plan` ⭐ · `day` · `night` · `transport`.

**Плюс два поза головним потоком**, бо це нові поверхні під нові підстави: **`guide`** (поверхня 8, job приїжджого) і **`safety`** (половина job'а 3/6, половина гіпотези H6).

**Головна вимога до макета, якої не було в попередньому наборі**: **превʼю формули гарантії на `route`, до генерації**. Доки плану немає, це єдине місце, де 2 видно взагалі.

**Три блоки складності на `route`**, кожен зі своєю підставою: факти (км, набір, найважчий день) · офіційна градація по ділянках **зі станом «не градуйовано»**, який є типовим випадком · навантаження за названим методом. Ніколи не одне число.
